...
#Plots of the System and its stability
count = 0
fig, ax = plt.subplots(3, 3, figsize=(20, 20))
h = [0.013, 0.095, 0.1705, 0.4231, 0.6112, 0.6435, 0.6693, 0.7115]
for i in range(3):
    for j in range(3):
        print(count)
        if count >= 8:
...
sol_dgl = odeint(DGL, [0.939, 0.24], t, args=(h[5], xi, pD, alpha, r, q, m, delta, kappa, v, b, s))

ax[1, 2].plot(sol_dgl[:, 0], sol_dgl[:, 1], color="black")
ax[1, 2].plot(sol_dgl[:, 0][0], sol_dgl[:, 1][0], 'x', color="black", label="Initial Condition")
fig.delaxes(ax[2, 2])

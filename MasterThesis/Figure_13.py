...
#Plots of System and its stability
count = 0
fig, ax = plt.subplots(3, 4, figsize=(20, 20))
h = [0.168, 0.274, 0.328, 0.371, 0.59, 0.732, 1.4, 1.481, 1.506, 1.6]
for i in range(3):
    for j in range(4):
        print(count)
        if count >= 10:
...
sol_dgl_0 = odeint(DGL, [0.227, 0.417], t, args=(h[2], xi, pD, alpha, r, q, m, delta, kappa, v, b, s))
sol_dgl_1 = odeint(DGL, [1, 0.6], t, args=(h[3], xi, pD, alpha, r, q, m, delta, kappa, v, b, s))
sol_dgl_2 = odeint(DGL, [1, 0.2], t, args=(h[4], xi, pD, alpha, r, q, m, delta, kappa, v, b, s))
sol_dgl_3 = odeint(DGL, [1, 0.6], t, args=(h[5], xi, pD, alpha, r, q, m, delta, kappa, v, b, s))
sol_dgl_4 = odeint(DGL, [0.752, 0.55], t, args=(h[6], xi, pD, alpha, r, q, m, delta, kappa, v, b, s))
sol_dgl_5 = odeint(DGL, [0.944, 0.597], t, args=(h[7], xi, pD, alpha, r, q, m, delta, kappa, v, b, s))
ax[0, 2].plot(sol_dgl_0[:, 0], sol_dgl_0[:, 1], color="black")
ax[0, 2].plot(sol_dgl_0[:, 0][0], sol_dgl_0[:, 1][0], 'x', color="black", label="Initial Condition")
ax[0, 3].plot(sol_dgl_1[:, 0], sol_dgl_1[:, 1], color="black")
ax[0, 3].plot(sol_dgl_1[:, 0][0], sol_dgl_1[:, 1][0], 'x', color="black", label="Initial Condition")
ax[1, 0].plot(sol_dgl_2[:, 0], sol_dgl_2[:, 1], color="black")
ax[1, 0].plot(sol_dgl_2[:, 0][0], sol_dgl_2[:, 1][0], 'x', color="black", label="Initial Condition")
ax[1, 1].plot(sol_dgl_3[:, 0], sol_dgl_3[:, 1], color="black")
ax[1, 1].plot(sol_dgl_3[:, 0][0], sol_dgl_3[:, 1][0], 'x', color="black", label="Initial Condition")
ax[1, 2].plot(sol_dgl_4[:, 0], sol_dgl_4[:, 1], color="black")
ax[1, 2].plot(sol_dgl_4[:, 0][0], sol_dgl_4[:, 1][0], 'x', color="black", label="Initial Condition")
ax[1, 3].plot(sol_dgl_5[:, 0], sol_dgl_5[:, 1], color="black")
ax[1, 3].plot(sol_dgl_5[:, 0][0], sol_dgl_5[:, 1][0], 'x', color="black", label="Initial Condition")
fig.delaxes(ax[2, 2])
fig.delaxes(ax[2, 3])

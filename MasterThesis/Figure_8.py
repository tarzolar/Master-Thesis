...
h = [0.02, 0.1, 0.4072, 0.7509, 1.037, 1.05, 1.181]
for i in range(3):
    for j in range(3):
        print(count)
        if count >= 7:
...
sol_dgl_1 = odeint(DGL, [1, 0.6], t, args=(h[4], xi, pD, alpha, r, q, m, delta, kappa, v, b, s))
...
ax[1, 1].plot(sol_dgl_1[:, 0], sol_dgl_1[:, 1], color="black")
ax[1, 1].plot(sol_dgl_1[:, 0][0], sol_dgl_1[:, 1][0], 'x', color="black", label="Initial Condition")
